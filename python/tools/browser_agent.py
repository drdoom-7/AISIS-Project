...
class State:
    ...
    def __init__(self, agent: Agent):
        ...
        self.iter_no = 0
        self.auto_download_extensions: List[str] = [".pdf"] # Removed image extensions for controlled download
    ...
    async def _save_file(self, filename_suggestion: str, content_bytes: Optional[bytes] = None, download_obj: Optional[PlaywrightDownload] = None, source_url: str = "Unknown"):
        ...
        try:
            ...
            PrintStyle().info(f"Successfully saved: {Path(save_path).name} to {self.downloads_path} (from URL: {source_url})")
    ...
    def start_task(self, task: str):
        ...
        async def run_wrapper(task_str):
            ...
            await self._write_debug_log(f"DEBUG: _run_task: Starting task with message: {task_str[:50]}...")
    ...
    async def _run_task(self, task: str):
        ...
        # New action to extract/download a specific image by selector
        ...
        async def extract_image_by_selector(params: ExtractImageParams):
            ...
            try:
                ...
                if not suggested_filename: # Fallback if URL has no filename
                    suggested_filename = f"extracted_image_{uuid.uuid4().hex}{Path(image_url).suffix or '.bin'}"

                # Save the image
                try:
                    await self._write_debug_log(f"DEBUG: extract_image_by_selector: Calling _save_file with filename_suggestion={suggested_filename}, content_bytes_len={len(content_bytes)}, source_url={image_url}")
                    saved_path = await self._save_file(
                        filename_suggestion=suggested_filename,
                        content_bytes=content_bytes,
                        source_url=image_url
                    )
                    await self._write_debug_log(f"DEBUG: extract_image_by_selector: _save_file returned: {saved_path}")
                except Exception as log_e:
                    PrintStyle().error(f"Error writing debug log in extract_image_by_selector: {log_e}")
                    # Continue without logging, but at least we know logging failed
                    saved_path = await self._save_file(
                        filename_suggestion=suggested_filename,
                        content_bytes=content_bytes,
                        source_url=image_url
